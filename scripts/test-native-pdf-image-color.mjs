...
async function testColors(functions) {
  ...
  const strictColors = new NativePdfColorRegistry(document, undefined, { iccEngine: "none" });
  const strictIcc = await strictColors.add([{ kind: "name", value: "ICCBased" }, ref(11)]);
  assert.throws(() => strictColors.convertToSrgb(strictIcc, [0, 0, 0]),
