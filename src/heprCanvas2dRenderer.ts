...
export class HeprCanvas2dBackend implements HeprDisplayBackend {
  ...
  private drawGradientResource(
    ...
    try {
      ...
      if (paintBackground && metadata.background !== null) {
  ...
  private convertColorToSrgb(index: number, components: ArrayLike<number>, path: string): readonly [number, number, number] {
    try {
      this.colorEvaluator ??= new HeprColorEvaluator(this.page.stores.colors,
        (indices, inputs, sourcePath) => this.evaluateFunctions(indices, inputs, sourcePath), this.options.signal);
      return this.colorEvaluator.convert(index, components, path);
    } catch (cause) {
      this.options.signal?.throwIfAborted();
      if (cause instanceof HeprCanvas2dError) throw cause;
      throw canvasError(HEPR_CANVAS_2D_ERROR_CODES.UnsupportedColor,
        cause instanceof Error ? cause.message : String(cause), path, cause);
    }
  }
