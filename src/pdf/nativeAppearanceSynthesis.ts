...
interface AnnotationBorderStyle extends BorderStyle {
...
export class NativePdfAppearanceSynthesizer {
  ...
  /**
   * ISO 32000-1 12.5.6.8. The square is drawn inside /Rect, inset by /RD and
  ...
  private async synthesizeSquare(
    ...
    if (!geometry) return null;
    const rectangle = await this.readSquareRectangle(annotation, geometry, signal);
    const width = rectangle[2] - rectangle[0];
    const height = rectangle[3] - rectangle[1];
    if (border.width > Math.min(width, height)) {
  ...
  private async readSquareRectangle(
    ...
    const raw = await this.document.resolveValue(annotation.dictionary.get("RD"), signal);
    if (raw === undefined || raw === null) return geometry.bbox;
    if (!Array.isArray(raw) || raw.length !== 4) {
      throw invalidBorder(annotation, "Square /RD must contain four rectangle differences.");
    }
    const differences: number[] = [];
    for (const item of raw) {
      const difference = await this.document.resolveValue(item, signal);
      if (typeof difference !== "number" || !Number.isFinite(difference) || difference < 0) {
        throw invalidBorder(annotation, "Square /RD must contain finite nonnegative differences.");
      }
      differences.push(difference);
    }
    // /RD uses left, top, right, bottom order, not rectangle coordinate order.
    const [left, top, right, bottom] = differences;
    if (left + right >= geometry.width || top + bottom >= geometry.height) {
      throw invalidBorder(annotation, "Square /RD leaves no area inside its annotation rectangle.");
    }
    return Object.freeze([left, bottom, geometry.width - right, geometry.height - top]);
  }
